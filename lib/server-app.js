...
// Stats par match d'un joueur NBA sur la saison en cours. L'endpoint roster n'en
// fournit aucune : il faut interroger l'athlète un par un.
async function fetchNbaPlayerStats(athleteId, season) {
  const cached = nbaStatsCache.get(athleteId)
  if (cached && Date.now() - cached.ts < NBA_STATS_TTL) return cached.data
  try {
    const r = await fetch(`https://sports.core.api.espn.com/v2/sports/basketball/leagues/nba/seasons/${season}/types/2/athletes/${athleteId}/statistics`)
    if (!r.ok) return null
    const data = await r.json()
    const raw = {}
    for (const c of data.splits?.categories || []) {
...
app.post('/api/challenge-score', async (req, res) => {
  const { email, points, correct } = req.body || {}
  if (!email) return res.status(400).json({ error: 'missing_email' })
  const result = await saveChallengeScore({ email, points, correct })
